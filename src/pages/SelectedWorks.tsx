...
const projects = [
    {
        id: "001",
        title: "Digital Learning & Training Platform",
        stack: "React / Node.js / MongoDB / Firebase",
        description:
            "A full-stack education platform built for IT training, course management, student learning, and digital career development.",
        links: {
    ...
    },
    {
        ...
        stack: "React / Node.js / MongoDB / Firebase",
        description:
            "A scalable e-commerce platform connecting online sales, POS, inventory, payments, and logistics through a unified business management system.",
        links: {
            live: "https://www.blanxer.com",
            code: "#",
        },
        image: "./p2.png",
        cta: "Live Project",
    ...
    {
        id: "003",
        title: "Blue Spark Diamond",
        stack: "React / Node.js / JavaScript / Tailwind CSS",
        description:
            "A responsive corporate website showcasing diamond products, global operations, certification, and the complete diamond sourcing and grading journey.",
        links: {
    ...
    },
    {
        ...
        description:
            "A modern education platform showcasing UK degree programmes, admissions, scholarships, student life, and career-focused learning.",
        links: {
            live: "https://iic.edu.np/",
            code: "#",
        },
        image: "./p4.png",
        cta: "Live Project",
...
const SelectedWorks = () => {
    ...
    const voidContainerRef =
        useRef<HTMLDivElement | null>(null);
    const kineticWheelRef =
        useRef<HTMLDivElement | null>(null);
    const threadPathRef =
        useRef<SVGPathElement | null>(null);
    const figureGroupRef =
        useRef<SVGGElement | null>(null);
    const threadLenRef = useRef<number>(0);

    // Refs for Typography animation
    const textAnalyzeRef =
        useRef<SVGTextElement | null>(null);
    const textDesignRef =
        useRef<SVGTextElement | null>(null);
    const textBuildRef =
        useRef<SVGTextElement | null>(null);
    const textDeliverRef =
        useRef<SVGTextElement | null>(null);
    ...
    useLenis(({ scroll }) => {
        ...
        const scaleEndPositionPx =
            stackPositionPx -
            (BASE_CONFIG.stackPosition -
                BASE_CONFIG.scaleEndPosition) *
            containerHeight;

        const lastCardTop =
            cardOffsets[cards.length - 1];

        const triggerEndLast =
            lastCardTop - scaleEndPositionPx;
        ...
        if (scroll > voidStart) {
            ...
            voidProgress = Math.min(
                Math.max(voidProgress, 0),
                1,
            );
        }
        ...
        for (let i = 0; i < cards.length; i++) {
            ...
            const triggerStart =
                cardTop -
                stackPositionPx -
                BASE_CONFIG.itemStackDistance * i;
            ...
            scaleProgress = Math.min(
                Math.max(scaleProgress, 0),
                1,
            );

            const targetScale =
                BASE_CONFIG.baseScale +
                i * BASE_CONFIG.itemScale;
            ...
            card.style.transform = `translate3d(0, ${Math.round(translateY * 10) / 10
                }px, 0) scale(${scale})`;
        }
            ...
            if (voidProgress > 0) {
                if (isMobile) {
                    ...
                    if (fadeOutProgress >= 1) {
                        voidContainer.style.visibility =
                            "hidden";
                    } else {
                        voidContainer.style.visibility =
                            "visible";
                    ...
                    voidContainer.style.transform = `translate3d(0, 0, ${currentZ}px) scale(${Math.max(
                        0,
                        currentScale,
                    ).toFixed(4)})`;
        ...
        if (
            ...
            const animateText = (
                ...
                ref.current.style.transformBox =
                    "fill-box";
            };
        ...
        if (kineticWheel) {
            if (
                ...
                kineticWheel.style.display =
                    "none";

                kineticWheel.style.visibility =
                    "hidden";
                ...
                if (isMobile) {
                    ...
                    kineticWheel.style.opacity =
                        figOpacity.toFixed(3);

                    kineticWheel.style.transform =
                        "translate3d(0, 0, 0)";

                    if (
                        figureGroupRef.current
                    ) {
                        if (
                            voidProgress >=
                            0.8
                            ...
                            figureGroupRef.current.style.opacity =
                                Math.max(
                                    0,
                                    textFade,
                                ).toFixed(3);
                    ...
                    kineticWheel.style.transformOrigin =
                        "50% 100%";
    ...
    const cachePositions = useCallback(() => {
        ...
        if (kineticWheelRef.current) {
            kineticWheelRef.current.style.transform =
                "";
        }
        ...
        if (
            ...
            try {
                const len =
                    threadPathRef.current.getTotalLength();

                if (len > 0) {
                    threadLenRef.current =
                        len;

                    threadPathRef.current.style.strokeDasharray =
                        `${len}`;

                    threadPathRef.current.style.strokeDashoffset =
                        `${len}`;
                }
            } catch {
        ...
        if (stackInnerRef.current) {
            stackInnerTopRef.current =
                stackInnerRef.current.getBoundingClientRect()
                    .top + scrollY;
        }
    ...
    useEffect(() => {
        ...
        cards.forEach((card, i) => {
            if (i < cards.length - 1) {
                card.style.marginBottom = `${BASE_CONFIG.itemDistance}px`;
            }
                    ...
                    <div className="marquee-selected-works__track">
                        {[0, 1, 2, 3].map(
                            (blockIndex) => (
                                ...
                                >
                                    <span className="marquee-selected-works__text">
                                        Selected
                                        Works
                                    </span>

                                    <span className="marquee-selected-works__dash">
                                        —
                                    </span>
                                </div>
                            ),
